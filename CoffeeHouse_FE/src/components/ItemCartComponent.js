...
export default function ItemCartComponent({ product, update }) {
  ...
  function changeQuantity(status) {
    if (status === "increase") {
      ...
      onUpdate(product.product.id, quantity + 1, checked);
  ...
  function onUpdate(productId, quantity, selected) {
    if (update) {
      update(productId, quantity, selected);
    }
  }
      ...
      >
        <View style={{ justifyContent: "center" }}>
          <Checkbox
            ...
            onPress={() => {
              ...
              onUpdate(product.product.id, quantity, selected);
              setChecked(!checked);
            }}
          />
        </View>
        ...
        <View style={{ padding: 15, flex: 2, justifyContent: "space-around" }}>
          ...
          <Text style={{ fontSize: 16, paddingTop: 7 }}>
            {Number(product.product.price).toLocaleString("it-IT", {
              style: "currency",
              currency: "VND",
            })}
          </Text>
